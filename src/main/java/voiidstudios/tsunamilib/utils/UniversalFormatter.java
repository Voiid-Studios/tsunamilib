...
public class UniversalFormatter {
    ...
    public UniversalFormatter(YALogger logger) {
        this.logger = logger;
    }
    ...
    private void warnFallback(String message, Throwable throwable) {
        ...
        logger.warning(message + ", so falling back to the legacy format: " + throwable.getClass().getSimpleName() + ": " + throwable.getMessage());
    }
}
